  ...
  methods: {
    ...
    async submitQuiz() {
      ...
      for (const q of this.questions) {
        try {
          const res = await fetch("https://iitmod-lifeskills.onrender.com/api/v1/questions/submit", {
            method: "POST",
            headers: {
          ...
          if (res.ok) {
            ...
            if (result.status === "CORRECT") {
              correct++;
              xp += q.reward_experience;
              money += q.reward_money;
            }
          } else if (res.status === 400) {
            console.warn(`❌ Incorrect answer for: ${q._id}`);
          } else {
            console.warn("❌ Error submitting question:", q._id);
          }
        } catch (error) {
