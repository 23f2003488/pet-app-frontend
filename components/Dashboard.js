...
<div class="top-icons">
  ...
  <button class="help-button" @click="goToHelp" title="Help">❓</button>
      ...
      <!-- Left Panel: Hunger, Happiness, Shop -->
...
<i class="fas fa-power-off"></i>
  ...
  methods: {
    async handleLogout() {
      ...
      try {
        // Hit backend logout API to terminate session
        ...
        console.warn("⚠️ Logout request failed (maybe already expired):", error.message);
      }
    ...
    playBark() {
      ...
      if (sound) {
        sound.currentTime = 0;
        sound.play().catch((e) => {
          console.log("Autoplay blocked or not supported:", e);
        });
      }
    },
  },
      ...
      fetch("https://iitmod-lifeskills.onrender.com/api/v1/users/get", {
        ...
        .catch((error) => {
          ...
          if (!retry && (error.message.includes("NetworkError") || error.message.includes("Failed to fetch"))) {
            console.log("⏳ Retrying once due to network issue...");
            setTimeout(() => fetchUserData(true), 500); // retry after 500ms
          } else {
